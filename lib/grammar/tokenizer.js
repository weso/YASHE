...
let $ = require('jquery');
const { keywords } = require("./_tokenizer-table.js");
...
CodeMirror.defineMode("shex", function(config, parserConfig) {
 ...
 var KW_ABSTRACT = '(a|A)(b|B)(s|S)(t|T)(r|R)(a|A)(c|C)(t|T)';
 var A_TOKEN = 'a';
 ...
 // var ATPNAME_NS = "@[a-zA-Z]*:";
  ...
  //  var SINGLELINE_COMMENT = "#([^\\n\\r]*[\\n\\r]|[^\\n\\r]*$)";
  var SINGLELINE_COMMENT = "#(.*[^\\/\\*]$)";
  var START_MULTILINE_COMMENT = "\\/\\*";
  var FINISH_MULTILINE_COMMENT ="\\*\\/";
  ...
  var terminals = [
    ...
    {
      name: "COMMENT",
      regex: new RegExp("^" + SINGLELINE_COMMENT),
      style: "comment"
    },
    
    
    {
      name: "START_MULTILINE_COMMENT",
      regex: new RegExp("^" + START_MULTILINE_COMMENT),
      style: "comment"
    },
    {
      name: "FINISH_MULTILINE_COMMENT",
      regex: new RegExp("^" + FINISH_MULTILINE_COMMENT),
      style: "comment"
    },
    {
      name: "IRI_REF",
  ...
  function tokenBase(stream, state) {
    ...
    function setSideConditions(topSymbol) {
      ...
      if (topSymbol === "baseDecl") {
        state.inBaseDecl = true;
      } else {
        state.inBaseDecl = false;
      }

      if (topSymbol === "importDecl") {
        state.inImportDecl = true;
      } else {
        state.inImportDecl = false;
      }
    ...
    if (tokenOb.cat == "<invalid_token>") {
      // We don´t mind if it´s invalid in case it´s inside a comment
      if(state.isMultiComment){
        state.OK = true;
        tokenOb.cat = "COMMENT";
        tokenOb.style = "comment";
    ...
    /**
     * In case we detect a multiline token start we set the state object isMultiComment to true.
     * In case of multiline token finish we set it to false.
     * When YASHE detects that the isMultiComment obj is true ignores everithing an sets all to comment.
     */
    switch(tokenOb.cat){
      case "START_MULTILINE_COMMENT":
        state.isMultiComment=true;
        state.possibleCurrent = state.possibleNext;
        return tokenOb.style;
      case "FINISH_MULTILINE_COMMENT":
        if(!state.isMultiComment)state.OK=false;
        state.isMultiComment=false;
        state.possibleCurrent = state.possibleNext;
        return tokenOb.style;
      default:
        if(state.isMultiComment){
           // we don´t allow regular expressions that contains */ inside comments.
           // Could lead with conflicts (e.g /*    /    */)
          if(tokenOb.cat=="REGEXP" && tokenOb.text.endsWith('*/')){
            state.isMultiComment=false
          }
          tokenOb.cat = "COMMENT";
          tokenOb.style = "comment";
    ...
    if (!tokenOb.quotePos || tokenOb.quotePos == "end") {
      ...
      while (state.stack.length > 0 && token && state.OK && !finished) {
        ...
        // Shaperef (e.g. @:User)
        if(tokenOb.cat=="ATPNAME_LN"){
          let shapeLabel=tokenOb.text.slice(1,tokenOb.text.length);
          console.log(defShapes)
          let isDefined = Object.keys(defShapes).filter(shape=>shape==shapeLabel).length > 0
          if(!isDefined){
            state.OK = false;
            recordFailurePos();
            state.errorMsg = "Shape " + shapeLabel + " is not defined";
          }
        }

        if(topSymbol=='shapeExprLabel'){
          defShapes[tokenOb.text] = tokenOb
          tokenOb.style='shape';
        }
        ...
        if(topSymbol == 'stringFacet')tokenOb.style='facet';
          ...
          if (topSymbol == token) {
            if (state.inPrefixDecl) {
              if (topSymbol === "PNAME_NS" && tokenOb.text.length > 0) {
                ...
                tokenOb.style = 'prefixDelcAlias'; //This is really weird but with 'Decl' instead of 'Delc' doesn't work
              } else if (state.currentPnameNs !== undefined && tokenOb.text.length > 2) {
                tokenOb.style = 'prefixDelcIRI'; //Same as before
                //console.log(defPrefixes)
                defPrefixes[state.currentPnameNs] = tokenOb.text.slice(1, -1);
            ...
            if (state.inBaseDecl) {
               if (topSymbol === "IRI_REF"){
                  tokenOb.style = 'baseDecl';
               }
            }

            if (state.inImportDecl) {
               if (topSymbol === "IRI_REF"){
                  tokenOb.style = 'importDecl';
               }
            }
            ...
            if (!state.inPrefixDecl && (token === "PNAME_NS" || token === "PNAME_LN")) {
              ...
              //  console.log({prefNs:prefNs,defPrefixes:defPrefixes, isDefined:defPrefixes[prefNs]})
                if (!defPrefixes[prefNs] && ["bif", "xsd", "sql"].indexOf(prefNs) < 0) {
              //    console.log('entra')
             //     console.log({tokenOb:tokenOb,prefNs:prefNs,defPrefixes:defPrefixes, in:defPrefixes[prefNs]})
                  state.OK = false;
                  recordFailurePos();
