  ...
  function tokenBase(stream, state) {
    ...
    if (!tokenOb.quotePos || tokenOb.quotePos == "end") {
      ...
      while (state.stack.length > 0 && token && state.OK && !finished) {
        ...
        //Shape
        if(topSymbol=='shapeExprLabel'){ //tokenOb.text?
          state.shapes[tokenOb.text] = tokenOb.text;
          tokenOb.style='shape';
        }
        ...
        if(topSymbol == 'valueSetValue'){
          tokenOb.style='valueSet';
        }

        if(topSymbol=='cardinality'){
          tokenOb.style='cardinality';
        }

        if(topSymbol == 'xsFacet'){
          tokenOb.style='facet';

        }

        if(tokenOb.cat == 'ATPNAME_LN'){
          let atShape = tokenOb.text.slice(1,tokenOb.text.length);
          if (!state.shapes[atShape]) {
                  state.OK = false;
                  recordFailurePos();
                  state.errorMsg = "Shape '" + atShape + "' is not defined";
          }
         // console.log(state.shapes[atShape])
        }
